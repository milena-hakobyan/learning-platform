...
class DatabaseConnectionTest {
    ...
    static void initDatabase() throws Exception {
        ...
        ds.setUser("sa"); //default user for h2
        ...
        // since the syntax of postgres and h2 are a bit different, I created 2 new scripts tailored for h2 logic.
    ...
    public void testExecuteArgsUpdate() {
        databaseConnection.execute(
                "INSERT INTO users (user_name, first_name, last_name, email, user_role, password_hash, last_login) " +
                        "VALUES (?, ?, ?, ?, ?, ?, ?)",
                "mcyrus", "Miley", "Cyrus", "mcyrus@example.com", "student", "hashed123", Timestamp.valueOf("2025-06-02 12:00:00")
        );

        databaseConnection.execute(
                "UPDATE users SET email = ?, user_role = ? WHERE user_name = ?",
                "miley.cyrus@updated.com", "instructor", "mcyrus"
        );
        ...
        assertNotNull(result);
        assertEquals("miley.cyrus@updated.com - INSTRUCTOR", result);
    }
